...
namespace Infrastructure
{
    public static class Dependencies
    {
        public static void AddInfrastructureService(this IServiceCollection services, IConfiguration configuration)
        {
            ...
            services.AddScoped<MongoRepository<RecommendProducts>>(sp =>
            {
                var client = sp.GetRequiredService<IMongoClient>();
                var dbName = configuration["MongoDbVectorSettings:DatabaseName"];

                return new MongoRepository<RecommendProducts>(client, dbName);
            });
            ...
            services.AddScoped<StellarChatService>(sp =>
            {
                ...
                var Ihost = sp.GetRequiredService<IHostEnvironment>();
                kernelBuilder.Plugins.AddFromObject(new StellarChatServicePlugin(semanticProductSearchService, mongoDbService, Ihost));
                return new StellarChatService(kernelBuilder.Build());
            });
            services.AddScoped<ChatSummarizationService>();
            services.AddScoped<MessageService>();
            services.AddScoped<SemanticRecommendProductsService>();
            services.AddScoped<SemanticKernelRecommendProductsService>();
            //快取
