...
namespace Web.Services.ProductNs
{
    ...
    public class ProductPageService
    {
        ...
        public async Task<ProductPageViewModel> GetProductPageServiceData(int currentProductId, int userId)
        {
            ...
            var similarProductDiscounts = await _productsDiscountRepository.ListAsync(d =>
                d.SalesStartDate <= currentDate &&
                d.SalesEndDate >= currentDate &&
                similarProductIds.Contains(d.ProductId));

            var discountDictionary = similarProductDiscounts
                .GroupBy(d => d.ProductId)
                .ToDictionary(
                    g => g.Key,
                    g => g.Max(d => d.Discount)
                );

            var similarProductViewModels = similarProducts.Select(sp =>
            {
                var applicableDiscount = discountDictionary.ContainsKey(sp.ProductId)
                    ? discountDictionary[sp.ProductId]
                    : 1m;

                decimal salesPrice = Math.Round(sp.ProductPrice * applicableDiscount, 0);
                ...
                return new SimilarProduct
                {
                    ...
                    Discount = applicableDiscount < 1m ? $"-{(1 - applicableDiscount) * 100:0}%" : null,
                    SalePrice = salesPrice,
                    FormattedSalePrice = salesPrice == 0 ? "免費遊玩" : $"NT$ {Math.Round(salesPrice):N0}"

                };
            }).ToList();

            //推薦的產品
            var recommendProducts = await _semanticRecommendProductsService.GetRecommendationsAsync(currentProductId,product.Description);
            var recommendDapper = await _productPageQueryService.GetRecommendProduct(recommendProducts);
            var recommendProductsViewModels = recommendDapper
                .Select(sp => new SimilarProduct
                {
                    ...
                    Title = sp.Title,
                    Price = sp.Price,
                    FormattedPrice = sp.Price == 0 ? "免費遊玩" : $"NT$ {Math.Round(sp.Price):N0}",
                    Discount=sp.Discount > 0 ? $"-{sp.Discount:0}%" : null,
                    FormattedSalePrice = sp.FormattedPrice == 0 ? "免費遊玩" : $"NT$ {Math.Round(sp.FormattedPrice):N0}"
                }).ToList();
            ...
            var productPageData = new ProductPageViewModel()
            {
                ...
                RecommendedProductList = recommendProductsViewModels,
