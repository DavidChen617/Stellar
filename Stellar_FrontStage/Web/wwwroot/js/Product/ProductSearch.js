...
var { ref, onMounted, computed, onBeforeUnmount, watch, onUnmounted, nextTick } = Vue;
const App = {
    setup() {
        ...
        const updateByAllProduct = async (fetchedProducts) => {
            const existingProductIds = new Set(products.value.map(product => product.productID));
           
            const uniqueProducts = fetchedProducts.filter(product => !existingProductIds.has(product.productID));
   
            products.value.push(...uniqueProducts);
        ...
        const fetchProductDataByPage = async (page) => {

            const endpoint = `/api/ProductSearchAPI/getProductDataByPage/page/${page}/pageSize/10`;

            var data = await handleFetch(endpoint);

            updateByAllProduct(data);
        };
        ...
        const handleScroll = () => {
            ...
            const triggerPoint = documentHeight * 0.65;
          

            //從 documentHeight -200 改為 -300
            if (scrollY + windowHeight >= triggerPoint && !isLoading.value && products.value.length < totalCount.value && priceValue.value[0] === 0 && priceValue.value[1] === 10) {
                currentPage.value++;
                fetchProductDataByPage(currentPage.value);
            }
