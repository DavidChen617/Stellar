...
namespace Web.Controllers
{
    public class MemberController : Controller
    {
        ...
        {
            // 解碼參數以確保其正確處理
            var decodedParameter = HttpUtility.UrlDecode(encodingParameter);
            Console.WriteLine($"Received encodingParameter: {decodedParameter}");

            var isChecked = await _changeEmailService.VerifyCheckEmail(decodedParameter);
        ...
        public async Task<IActionResult> SendMail(int id, string address)
        {
            var loginUserId = Int32.Parse(HttpContext.User.FindFirst(ClaimTypes.Sid)?.Value ?? "0");

            var resultMessage = await _changeEmailService.SendCheckEmail(loginUserId, address);
            if (resultMessage == "Verification email sent successfully.")
            {
                return Json(new { success = true, message = "Verification email sent. Please check your inbox." });
            }
            else
            {
